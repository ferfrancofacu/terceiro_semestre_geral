...
Digite, por exemplo, a seguinte URL no Firefox:
    http://www.omdbapi.com/?s=star%20wars&apikey=de39793f

Observe que vemos uma lista de 10 filmes, mas há mais resultados.

Para ver a página 2, acesse
    http://www.omdbapi.com/?s=star%20wars&page=2&apikey=de39793f
'''


'''
Observe que nas URLs acima, estamos passando parâmetros.
Na URL http://www.omdbapi.com/?s=star%20wars&page=2&apikey=de39793f
...
Como fazer isso?
no final da URL colocar o filtro &type=movie
...
Resposta: nenhum / não existe o filtro jogos na docimentação
...
def busca_qtd_total(texto_buscar):
    return int(busca_por_texto(texto_buscar)['totalResults']) -1
...
def busca_qtd_filmes(texto_buscar):
    ...
    print(url)
    retorno = req.get(url).json()
    return int(retorno['totalResults']) -9
...
def busca_qtd_jogos(texto_buscar):
    url = "http://www.omdbapi.com/?apikey={}&s={}&type=games".format(api_key, texto_buscar)
    print(url)
    retorno = req.get(url).json()
    return int(retorno['totalResults'])
