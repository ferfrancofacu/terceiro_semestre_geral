...
def peca_valida(peca):
    ...
    if (peca.direita <= 6 and peca.direita > 0 and peca.esquerda <= 6 and peca.esquerda > 0):
        return isinstance(peca.direita, numbers.Integral) and isinstance(peca.esquerda, numbers.Integral)
    else:
       return False
...
def encaixa_de_algum_jeito(peca1,peca2):
    domino = [peca1.direita,peca1.esquerda]
    for i in domino:
        if i == peca2.direita:
            return True
        if i == peca2.esquerda:
...
def encaixa_girando_so_a_segunda(peca1,peca2):
    domino = [peca2.direita,peca2.esquerda]
    for i in domino:
        if i == peca1.direita:
            return True
        if i == peca1.direita:
...
def jogo_valido(jogo):
    ...
    colocar = False
    tamando = len(jogo)-1
    for d in range(tamando):

        if(d==0):
            primeiDomino = jogo[ctrl]
            ultimoDomino = jogo[ctrl2]
            colocar = encaixa(primeiDomino, ultimoDomino)
        else:
            proxDomino = jogo[ctrl2]
            colocar = encaixa(ultimoDomino,proxDomino)
            ultimoDomino = proxDomino
...
def ciclo_valido(jogo):
    ...
    return primeiDomino.esquerda == proxDomino.direita
...
def jogador(jogo,peca):   
    primeiraPeca = jogo[0].esquerda
    ultimaPeca = jogo[len(jogo)-1].direita

    if primeiraPeca == peca.direita or primeiraPeca == peca.esquerda:
        return 'atras'

    if ultimaPeca == peca.esquerda or primeiraPeca == peca.esquerda:
        return 'frente'
...
def jogo_grande(pecas):
    ...
    if ciclo_valido(pecas):
        return pecas,pecas_sobrando
    else:
        jogo_grande.append(pecas[0])

        for ctrl in range(1,len(pecas)):
            ultimaPeca = jogo_grande[len(jogo_grande)-1].direita
            proxPecaEsq = pecas[ctrl].esquerda
            provPecaDir = pecas[ctrl].direita
            
            if ultimaPeca == proxPecaEsq or ultimaPeca == provPecaDir:
                jogo_grande.append(pecas[ctrl])
            else:
                 pecas_sobrando.append(pecas[ctrl])   
