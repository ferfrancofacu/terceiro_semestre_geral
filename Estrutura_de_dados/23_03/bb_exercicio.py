...
'''
Exercicio: Escreva uma funcao que recebe dois numeros e devolve sua média
...
def passo_da_busca_binaria(lista,procurando,menor_i,maior_i):
    meio = numero_do_meio(lista,menor_i,maior_i)
    i_meio = media_arredondada(menor_i,maior_i)

    if meio < procurando:
        return (i_meio+1, maior_i)

    if meio > procurando:
        return (menor_i, i_meio-1)
    return meio == procurando
